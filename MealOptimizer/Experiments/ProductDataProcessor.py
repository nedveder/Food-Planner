...
class ProductDataProcessor:
    ...
    def create_smaller_dataset(self, sample_size, future_date = None):
        ...
        smaller_df = self.products_df.sample(n=sample_size).reset_index(drop=True)
...
def main():
    sample_size = 10  # Number of products in the smaller dataset
    future_date = '2024-12-31'  # Example future date
    processor = ProductDataProcessor(sample_size, future_date)

    # Create a smaller dataset with random dates
    smaller_dataset = processor.create_smaller_dataset(sample_size, future_date)
